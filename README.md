...
│   ├── room-service/         # Room 관리 마이크로서비스
│   ├── voice-agent/          # Python Voice AI Agent (STT/LLM/TTS)
│   └── web-client/           # Next.js 음성 대화 테스트 클라이언트
├── libs/
...
| 서비스 | URL | 설명 |
|--------|-----|------|
| Web Client | http://localhost:3002 | 음성 대화 테스트 UI |
| API Gateway | http://localhost:3000 | REST API |
| Swagger | http://localhost:3000/api | API 문서 |
| Grafana | http://localhost:3001 | 모니터링 대시보드 |
| Jaeger | http://localhost:16686 | 분산 트레이싱 |
| RabbitMQ | http://localhost:15672 | 메시지 브로커 관리 |
...
| **Observability** | OpenTelemetry, Jaeger, Loki, Grafana, Pino |
...
스코프: `api-gateway`, `livekit-service`, `room-service`, `web-client`, `core`, `redis`, `prisma`, `shared` 등
